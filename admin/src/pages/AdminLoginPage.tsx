  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 rounded-2xl overflow-hidden shadow-2xl border border-slate-800">
        {/* Brand panel (Hidden on small screens or adapted) */}
        <div className="hidden md:flex flex-col justify-between bg-gradient-to-br from-indigo-900 via-purple-900 to-violet-950 text-white p-8 sm:p-10 relative overflow-hidden">
        ...
        {/* Form panel optimized for mobile view */}
        <div className="bg-white p-6 sm:p-10 flex flex-col justify-center">
          {/* Mobile top brand indicator */}
          <div className="flex md:hidden items-center gap-2 text-[#49557E] font-bold text-lg mb-4">
            <BsTicketPerforated size={22} />
            <span>EventNest</span>
          </div>

          <h1 className="text-xl sm:text-2xl font-bold text-slate-800 mb-1">Admin Sign In</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
            ...
            <div>
              <label className="text-[11px] sm:text-xs text-slate-400 uppercase tracking-wide font-semibold">
                Email Address
              </label>
              <div className="mt-1.5 flex items-center gap-2 border border-slate-200 rounded-xl px-3 py-2.5 sm:py-3 focus-within:border-[#49557E] transition-colors">
                <BsEnvelope className="text-slate-400 flex-shrink-0" size={16} />
                <input
                  type="email"
                  required
                  value={email}
                  ...
                  className="w-full text-sm text-slate-800 outline-none placeholder:text-slate-300 bg-transparent"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1.5 flex items-center gap-2 border border-slate-200 rounded-xl px-3 py-2.5 sm:py-3 focus-within:border-[#49557E] transition-colors">
                <BsLock className="text-slate-400 flex-shrink-0" size={16} />
                <input
                  type="password"
                  required
                  value={password}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#49557E] hover:bg-[#3c4768] disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold text-sm py-3 rounded-full transition-all shadow-sm active:scale-95"
            >
              {loading ? "Signing in..." : "Sign In"}
            </button>
          </form>

          <p className="text-xs text-slate-400 mt-6 sm:mt-8 text-center">
            Not an admin?{" "}
