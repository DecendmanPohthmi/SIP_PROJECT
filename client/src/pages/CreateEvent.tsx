    ...
    <div className="max-w-3xl mx-auto mt-4 sm:mt-8 px-4 sm:px-6 pb-16">
      ...
      >
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex-1 py-3 rounded-lg border font-medium text-sm sm:text-base transition-colors ${
                  pricingMode === "free"
                    ? "bg-[#49557E] text-white border-[#49557E]"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#49557E] text-white py-3.5 rounded-xl font-semibold text-sm sm:text-base hover:bg-[#3b4566] disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors shadow-sm"
        >
...
export const EditEvent = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        ...
        setTotalCapacity(event.total_capacity ? String(event.total_capacity) : "");
        setAvailableCapacity(event.available_capacity ? String(event.available_capacity) : "");
        setPricingMode(event.pricing_mode || "paid");
        if (event.image_url) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("end_time", endTime);
      formData.append("total_capacity", totalCapacity);
      formData.append("available_capacity", availableCapacity || totalCapacity);
      formData.append("pricing_mode", pricingMode);
      if (imageFile) formData.append("image", imageFile);

      const res = await fetch(`${API}/api/events/update/${id}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
          token: token || "",
  ...
  return (
    <div className="max-w-3xl mx-auto mt-4 sm:mt-8 px-4 sm:px-6 pb-16">
      <div className="mb-6 sm:mb-8">
        ...
        <p className="text-sm sm:text-base text-slate-500 mt-1">
          Update your event details. Updated events may require re-approval.
        </p>
      </div>
      ...
      >
        ...
        {success && (
          <p className="text-emerald-600 text-sm font-medium">
            Event updated successfully! Redirecting to your dashboard...
          </p>
        )}
        ...
        <div className="flex items-center gap-3 pt-2">
          <button
            ...
            className="flex-1 py-3.5 border border-gray-300 text-slate-700 rounded-xl font-semibold text-sm sm:text-base hover:bg-slate-50 transition-colors"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={loading}
            className="flex-1 bg-[#49557E] text-white py-3.5 rounded-xl font-semibold text-sm sm:text-base hover:bg-[#3b4566] disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors shadow-sm"
          >
