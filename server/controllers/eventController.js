import {
  ...
  getTopEarningEventsByOrganiser,
...
export const cancelEventByOrganiser = async (req, res) => {
  try {
    ...
    if (!event) {
      return res.status(400).json({
        success: false,
        message: "Only your own valid/live events can be cancelled.",
      });
    }
...
export const removeEvent = async (req, res) => {
  try {
    ...
    // Strict Admin check: Organisers cannot delete events permanently, they can only cancel them.
    if (req.user.role !== "admin") {
      return res.status(403).json({
        success: false,
        message: "Deletion is restricted to administrators only. Please use cancel instead.",
      });
    }
...
export const getPlatformStats = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      totalOrganisers: stats.total_organisers,
...
export const getTopEarningEvents = async (req, res) => {
  try {
    const organiserId = req.user?.organiser_id || req.user?.user_id || req.user?.id;
    ...
    const events = await getTopEarningEventsByOrganiser(organiserId, 3);
    ...
    console.error("Error fetching top earning events:", error);
    return res.status(500).json({
      success: false,
      message: "Server Error",
    });
