...
// ================= GET ALL EVENTS (Admin / Management) =================
export const getAllEvents = async () => {
  const result = await pool.query(
    `SELECT 
       ...
       o.full_name, 
       o.organisation_name,
       COALESCE(SUM(b.quantity), 0)::INTEGER AS tickets_sold,
...
// ================= CANCEL EVENT (organiser: live/approved -> cancelled) =================

export const cancelEvent = async (event_id, organiser_id) => {
  const result = await pool.query(
    `UPDATE events SET status='cancelled', updated_at=CURRENT_TIMESTAMP
     WHERE event_id=$1 AND organiser_id=$2 AND status IN ('live', 'approved', 'pending')
     RETURNING *`,
    [event_id, organiser_id],
  );
  return result.rows[0];
...
export const getPlatformStatsFromDB = async () => {
  const result = await pool.query(
    `SELECT
      (SELECT COUNT(*)::INTEGER FROM organisers WHERE status = 'approved') AS total_organisers,
      (SELECT COUNT(*)::INTEGER FROM events WHERE status IN ('live', 'completed')) AS total_events,
      (SELECT COALESCE(SUM(quantity), 0)::INTEGER FROM bookings WHERE booking_status = 'confirmed') AS total_tickets,
      (SELECT COALESCE(SUM(total_amount), 0)::NUMERIC FROM bookings WHERE booking_status = 'confirmed') AS total_amount,
      (SELECT COUNT(*)::INTEGER FROM users) AS total_attendees`,
  );
  return result.rows[0];
};

export const getTopEarningEventsByOrganiser = async (organiserId) => {
  const query = `
    SELECT 
      e.event_id, 
      e.title, 
    ...
    GROUP BY e.event_id, e.title, e.category, e.image_url
    ORDER BY total_revenue DESC
    LIMIT 3;
  `;
  const { rows } = await pool.query(query, [organiserId]);
  return rows;
};